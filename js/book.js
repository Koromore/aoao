$(function () {
	...
	for (var i = 0; i < $('.runPage').length; i++) {
		$('.runPage').eq(i).css('z-index', 12 - 2 * i);
		$('.runPage').eq(i).children('div').css('z-index', 12 - 2 * i);
		$('.runPage').eq(i).children('img').css('z-index', 11 - 2 * i);
	};
	...
	function zIndexLast(index, element) {
		if (index == 0) {
			element.css('z-index', 12 - 2 * index);
		};
		setTimeout(function () {
			element.css('z-index', 12 - 2 * index);
			element.children('div').css('z-index', 12 - 2 * index);
